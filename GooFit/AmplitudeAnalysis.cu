...
*/
 ...
 Variable* massPsiPi = new Variable("massPsiPi",S(23),-1,1); // cosine of the K* helicity angle
 ...
 UnbinnedDataSet point(obserVariables);
 ...
 if(dataTxt.good()){
  ...
  TH1F mKPHistoTest("mKPHistoTest", "mKPHistoTest",datapoints1, massKPi->lowerlimit, massKPi->upperlimit);

  fptype roll=0.0; fptype rollSum=0.0; fptype rollAvg = 0.0;
  fptype diff = 0.0; fptype diffSum = 0.0; fptype diffAvg = 0.0;
	fptype func=0.0;
  fptype max = 0.1;
  ...
  for (int j = 0; j < events; ++j) {
        ...
        ++roundIter;
  			...
  			massKPi->value = ranGen.Uniform(massKPi->upperlimit-massKPi->lowerlimit)+massKPi->lowerlimit;
        cosMuMu->value = ranGen.Uniform(cosMuMu->upperlimit-cosMuMu->lowerlimit)+cosMuMu->lowerlimit;
        cosKstar->value = ranGen.Uniform(cosKstar->upperlimit-cosKstar->lowerlimit)+cosKstar->lowerlimit;
        phi->value = ranGen.Uniform(phi->upperlimit-phi->lowerlimit)+phi->lowerlimit;

  			func = matrix->getValue();
  			roll = ranGen.Uniform(max);

        /*rollSum += roll;
        rollAvg = (rollAvg * (((fptype)roundIter-1)/(fptype)roundIter)) + roll/((fptype)roundIter);

        diff = roll - func;
        diffSum += diff;
        diffAvg = (diffAvg * (((fptype)roundIter-1.)/(fptype)roundIter)) + diff/((fptype)roundIter);
        //std::cout<<"RoundJ :"<<roundIter<<"Funcion : "<<func<<"Roll : "<<roll<<" Roll avg : "<<rollAvg<<" Difference avg : "<<diffAvg<<std::endl;
        ...
        if (roll > func) {
  				--j;
          /*if(roundIter>1.25*roundJ && diffAvg>0.0) {
            max -= diffAvg*1.5;
            if (max<0) max+=diffAvg*0.25;
            if (max<0) max+=diffAvg*0.25;
            if (max<0) max+=diffAvg*0.25;
            if (max<0) max+=diffAvg*0.25;
            roundIter = 0;
            //std::cout<<"Max reduced to : "<<max<<" Generated : "<<roundJ<<" events "<<std::endl;
            rollSum = 0.0;
            diffAvg = 0.0;
  			...
  			// 	continue;}
        ++roundJ;
  			mKPHistoTest.Fill(massKPi->value);
        std::cout<<j<<" "<<std::endl;
  	}

  mKPHistoTest.Draw();
  canvas->SaveAs("generation.eps");
  canvas->Clear();
  ...
  UnbinnedDataSet gridData(obserVariables);
